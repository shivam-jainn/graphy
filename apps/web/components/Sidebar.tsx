  ...
  return (
    <aside className={`h-screen bg-background border-r transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-64' : 'w-16'} relative flex flex-col`}>
      ...
      <div className="flex-1 overflow-y-auto px-2 h-full">
        <SidebarItem icon={<FiMessageSquare />} label="Chat History" isOpen={isSidebarOpen} isExpandable={false}>
          <ChatHistory />
        </SidebarItem>

        <SidebarItem icon={<FiFile />} label="Uploads" isOpen={isSidebarOpen} hideTitle={true}>
          <Uploads />
        </SidebarItem>
      </div>
...
function SidebarItem({ icon, label, isOpen, children, isExpandable, hideTitle }: SidebarItemProps) {
  const [expanded, setExpanded] = useState(!isExpandable); // Default to expanded if not expandable
  ...
  return (
    <div className="relative">
     
     {
      !hideTitle ? "" :
      <div
      className={`flex items-center gap-2 p-2 rounded-lg hover:bg-muted transition-all cursor-pointer ${
    ...
    >
      ...
      {(expanded || !isExpandable) && isOpen && <div className="mt-2">{children}</div>}
      {!isOpen && (
